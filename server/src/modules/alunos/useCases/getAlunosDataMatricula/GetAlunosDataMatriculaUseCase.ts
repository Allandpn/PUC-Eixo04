...
export class GetAlunosDataMatriculaUseCase {
    async execute() : Promise<any>{
            ...
            try {
                ...
                const teste = await prisma.$queryRaw<any>`SELECT  al.id as idAluno, un.nome as nomeUnidade, al.created_at FROM ((unidades un INNER JOIN turmas tr ON un.id = tr.unidadeId) INNER JOIN alunos al ON al.turmaId = tr.id )`;
